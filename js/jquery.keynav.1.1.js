 ...
 *
 * 27/02/2011 Edited by molokoloco - only work with blur, no more class
 *
 */
(function ($) {
	$.keynav = new Object();
	$.fn.keynav = function () { //onClass, offClass) {
		...
		if (!$.keynav.init) {
			$.keynav.el = new Array();
			$(document).bind('keydown', $.keynav.keyNavEvent);
			$.keynav.init = true;
		}
	...
	/*$.fn.keynav_sethover = function (onClass, offClass) { // mlklc mod
		return this.each(function () {
			this.onClass = onClass;
			this.offClass = offClass;
		});
	}*/
	$.fn.keynavReset = function () { // mlklc mod
		$(document).unbind('keydown', $.keynav.keyNavEvent);
		$.keynav.el = null;
		$.keynav.init = false;
	}
	$.keynav.reg = function (e) { //, onClass, offClass) {
		e.pos = $.keynav.getPos(e);
		//e.onClass = onClass; // mlklc mod
		// e.offClass = offClass;
	...
	$.keynav.setActive = function (e) {
		...
		$(cur).trigger('blur');
		/* for (var i = 0; i < $.keynav.el.length; i++) { // mlklc mod
			var tmp = $.keynav.el[i];
	...
	$.keynav.quad = function (cur, fQuad) {
		var quad = Array();
		for (i = 0; i < $.keynav.el.length; i++) {
			var el = $.keynav.el[i];
			if (cur == el) continue;
			if (fQuad((cur.pos.cx - el.pos.cx), (cur.pos.cy - el.pos.cy))) quad.push(el);
		}
		return quad;
	}
	...
	$.keynav.goLeft = function () {
		...
		var quad = $.keynav.quad(cur, function (dx, dy) {
			if ((dy >= 0) && (Math.abs(dx) - dy) <= 0) return true;
			else
			return false;
		});
	...
	$.keynav.goUp = function () {
		var cur = $.keynav.getCurrent();
		var quad = $.keynav.quad(cur, function (dx, dy) {
			if ((dx >= 0) && (Math.abs(dy) - dx) <= 0) return true;
			else
			return false;
		});
		$.keynav.activateClosest(cur, quad);
